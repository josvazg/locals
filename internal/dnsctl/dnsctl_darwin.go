...
func (d *osDNSController) Grab() error {
	...
	if _, err := d.p.Run("sudo", "cp", tmpLocalsResolverFile, resolverMacLocalsFile); err != nil {
		return fmt.Errorf("failed to copy resolver config to %s: %w", resolverMacLocalsFile, err)
	}
	if _, err := d.p.Run("sudo", "chmod", "o+r", resolverMacLocalsFile); err != nil {
...
func (d *osDNSController) Release() error {
	...
	if _, err := d.p.Run("sudo", "rm", resolverMacLocalsFile); err != nil {
